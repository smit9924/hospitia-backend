...
def welcome_email_notification_exception_handler(
    *,
    original_queue: str,
    dead_letter_queue: str,
    data: dict[str, Any],
    ...
    """
    Requeue or dead-letter a failed welcome email notification.
    ...
    payload: dict[str, Any] = dict(data)
    retry_count = int(payload.get("retry_count") or 0)
    max_retry_count = settings.WELCOME_EMAIL_RETRY_COUNT
    ...
    if retry_count < max_retry_count:
        ...
        mq_client.publish(original_queue, payload)
        log.warning(
            "Requeued welcome email notification "
            "(retry_count=%s/%s): %s",
            payload["retry_count"],
            max_retry_count,
            exc,
        ...
        mq_client.publish(dead_letter_queue, payload)
        log.error(
            "Moved welcome email notification to dead letter queue "
            "(retry_count=%s): %s",
            retry_count,
            exc,
        )
